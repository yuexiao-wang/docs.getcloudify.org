...
yaml_link: http://getcloudify.org/spec/puppet-plugin/1.3.1/plugin.yaml
---
...
Note that you don't have to provide manifests, you can provide Puppet code directly in the manifest under `properties` > `puppet_config` > OPNAME > `execute`, see the example in [Per-operation Puppet manifests or code to execute](#per-operation-puppet-manifests-or-code-to-execute).
...
**Derived from:** [cloudify.nodes.WebServer]({{< relref "blueprints/built-in-types.md" >}})
...
When defining a YAML node, there are several places that contain per-operation configuration. Most of the operations in Cloudify are named `cloudify.interfaces.lifecycle.*` and `cloudify.interfaces.relationship_lifecycle.*`.

For convenience, when defining a node, the operation names are shortened so only the last part is used.
...
The following Cloudify-specific facts exist for all operations.
...
### Operations with Two Nodes

The following Cloudify-specific facts exist for all operations involving two-nodes.
...
### Example Setting Puppet Version
...
### Example Specifying Facts in a Blueprint
...
### Example Passing Node Properties as Facts:
...
### Example downloading Manifests from URL
...
### Example Embedding Manifests in Blueprint
...
### Example of Generated Puppet Node Name
...
### Example of User-Specified Puppet Node Name
...
### Example Showing Puppet Module Installation
...
### Example with Per-Operation Puppet Execution
